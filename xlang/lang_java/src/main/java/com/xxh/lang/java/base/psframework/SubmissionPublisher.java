...
*/
/**
 * A {@link Publisher} that asynchronously issues submitted
 ...
 * Subscription#request(long) request} do not directly result in
 ...
 * {@link Subscriber#onNext onNext}, but exceptions in methods
 * {@link Subscriber#onSubscribe onSubscribe},
 * {@link Subscriber#onError(Throwable) onError} and
 * {@link Subscriber#onComplete() onComplete} are not recorded or
...
/**
     ...
     * Subscriber#onNext(Object) onNext}.
     *
...
/**
     ...
     * method {@link Subscriber#onNext(Object) onNext}.
     *
     * @param executor the executor to use for async delivery,
     * supporting creation of at least one independent thread
     * @param maxBufferCapacity the maximum capacity for each
...
/**
     ...
     * Subscriber#onError(Throwable) onError} method is invoked on
     * the existing subscription with an {@link IllegalStateException}.
     * Otherwise, upon success, the Subscriber's {@link
     * Subscriber#onSubscribe onSubscribe} method is invoked
     * asynchronously with a new {@link Subscription}.  If {@link
     * Subscriber#onSubscribe onSubscribe} throws an exception, the
     * subscription is cancelled. Otherwise, if this SubmissionPublisher
     * was closed exceptionally, then the subscriber's {@link
     * Subscriber#onError onError} method is invoked with the
     * corresponding exception, or if closed without exception, the
     * subscriber's {@link Subscriber#onComplete() onComplete}
     * method is invoked.  Subscribers may enable receiving items by
     * invoking the {@link Subscription#request(long) request}
     * method of the new Subscription, and may unsubscribe by invoking
     * its {@link Subscription#cancel() cancel} method.
     *
...
/**
     ...
     * asynchronously invoking its {@link Subscriber#onNext(Object)
...
/**
     ...
     * Subscriber#onNext(Object) onNext} method. The item may be
     ...
     * Subscriber#onError(Throwable) onError} signal to the
     * subscriber.
     *
...
/**
     * Publishes the given item, if possible, to each current subscriber
     * by asynchronously invoking its {@link
     * Subscriber#onNext(Object) onNext} method, blocking while
     ...
     * error and/or issuing an {@link Subscriber#onError(Throwable)
...
/**
     ...
     * Subscriber#onComplete() onComplete} signals to current
...
/**
     * Unless already closed, issues {@link
     * Subscriber#onError(Throwable) onError} signals to current
...
/**
     ...
     * tracking purposes, not for invoking {@link Subscriber}
...
/**
     ...
     * (via {@link Subscription#request(long) request}) but not
...
/**
     ...
     * publisher signals {@link Subscriber#onComplete()
